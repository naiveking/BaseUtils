package com.king.naiveutils.utils.bar;
...
/**
 * 系统UI Info
 * Created by NaiveKing on 2022/1/17
 */
public class OSInfo {
    public enum OSType {
        OS_TYPE_OTHER(0),
    ...
    public static OSInfo.OSType getRomType(Context context) {
        ...
        if (osTypeValue == -1) {
            ...
            if (display.toUpperCase().contains("FLYME")) {
                sharedPreferences.edit().putInt("os_type", OSInfo.OSType.OS_TYPE_FLYME.value).commit();
                return OSInfo.OSType.OS_TYPE_FLYME;
            } else {
                if (!TextUtils.isEmpty(getProp(KEY_VERSION_MIUI))) {
                    sharedPreferences.edit().putInt("os_type", OSInfo.OSType.OS_TYPE_MIUI.value).commit();
                    return OSInfo.OSType.OS_TYPE_MIUI;
                } else if (!TextUtils.isEmpty(getProp(KEY_VERSION_EMUI))) {
                    sharedPreferences.edit().putInt("os_type", OSInfo.OSType.OS_TYPE_EMUI.value).commit();
                    ...
                    sharedPreferences.edit().putInt("os_type", OSInfo.OSType.OS_TYPE_COLOR.value).commit();
                    return OSInfo.OSType.OS_TYPE_COLOR;
                } else if (!TextUtils.isEmpty(getProp(KEY_VERSION_FUNTOUCH))) {
                    sharedPreferences.edit().putInt("os_type", OSInfo.OSType.OS_TYPE_FUNTOUCH.value).commit();
                    return OSInfo.OSType.OS_TYPE_FUNTOUCH;
                } else {
                    sharedPreferences.edit().putInt("os_type", OSInfo.OSType.OS_TYPE_OTHER.value).commit();
            ...
            switch (osTypeValue) {
                case 0:
                    osType = OSInfo.OSType.OS_TYPE_OTHER;
                    break;
                case 1:
                    osType = OSInfo.OSType.OS_TYPE_EMUI;
                    break;
                case 2:
                    osType = OSInfo.OSType.OS_TYPE_MIUI;
                    break;
                case 3:
                    osType = OSInfo.OSType.OS_TYPE_FLYME;
                    break;
                case 4:
                    osType = OSInfo.OSType.OS_TYPE_COLOR;
                    break;
                case 5:
                    osType = OSInfo.OSType.OS_TYPE_FUNTOUCH;
                    break;
                default:
                    osType = OSInfo.OSType.OS_TYPE_OTHER;
                    break;
